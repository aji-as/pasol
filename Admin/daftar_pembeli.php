    ...
    <?php if ($_GET['status'] === "ok"): ?>
        <div id='success-alert' 
            ...
            <div class='flex-1'>
                ...
                <p class='text-sm text-gray-500 mt-0.5'>Pembeli dihapus.</p>
            </div>
...
<div class="p-4 md:p-8 bg-gray-50 min-h-screen font-sans">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        ...
        <div class="flex flex-col sm:flex-row gap-3 w-full md:w-auto">
            ...
            <a href="./?page=tambah-pembeli" class="w-full sm:w-auto flex items-center justify-center gap-2 bg-green-600 text-white px-5 py-2.5 rounded-lg font-semibold text-sm hover:bg-green-700 transition shadow-md">
    ...
    $buyers = get_user_by_role("pembeli");
    ?>

    <div class="grid grid-cols-1 gap-4 md:hidden">
        <?php foreach($buyers as $buyer) {
            // Persiapan Data
            $namaUrl = urlencode($buyer['nama_lengkap']); 
            $avatarSrc = "https://ui-avatars.com/api/?name={$namaUrl}&background=00A651&color=fff&size=128";
            $tanggalGabung = date('d M Y', strtotime($buyer['created_at'])); // Format Tanggal
        ?>
        ...
        <div class="bg-white p-5 rounded-2xl shadow-sm border border-gray-200 transition-shadow hover:shadow-md">
            ...
            <div class="flex items-start gap-4 mb-4">
                ...
                <div class="flex-1 min-w-0"> <div class="flex justify-between items-start">
                        <h3 class="font-bold text-gray-900 text-lg leading-tight truncate">
                            <?php echo $buyer['nama_lengkap']; ?>
                        </h3>
                        <span class="text-[10px] font-mono bg-gray-100 text-gray-500 px-2 py-0.5 rounded-full">
                            #<?php echo $buyer['id']; ?>
                        </span>
                    </div>
                    <p class="text-sm text-gray-500 truncate"><?php echo $buyer['email']; ?></p>
                </div>
            </div>
            ...
            <div class="bg-gray-50 rounded-lg p-3 mb-4 space-y-2 border border-gray-100">
                <div class="flex items-center text-sm text-gray-700">
                    ...
                    <span class="font-medium"><?php echo $buyer['no_hp']; ?></span>
                </div>
            ...
            <div class="grid grid-cols-3 gap-2 border-t border-gray-100 pt-3">
                <a href="./?page=detail-user&id=<?php echo $buyer['id']; ?>" class="flex items-center justify-center gap-1 py-2 text-blue-600 bg-blue-50 rounded-lg text-xs font-bold hover:bg-blue-100 transition group">
                    <svg class="w-4 h-4 group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                    Detail
                </a>
                
                <a href="./?page=edit-user&id=<?php echo $buyer['id']; ?>" class="flex items-center justify-center gap-1 py-2 text-yellow-700 bg-yellow-50 rounded-lg text-xs font-bold hover:bg-yellow-100 transition group">
                    <svg class="w-4 h-4 group-hover:scale-110 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                    Edit
                </a>
                
                <button onclick="confirmDelete(<?php echo $buyer['id']; ?>, '<?php echo $buyer['nama_lengkap']; ?>')" class="flex items-center justify-center gap-1 py-2 text-red-600 bg-red-50 rounded-lg text-xs font-bold hover:bg-red-100 transition group">
                ...
                <tr class="hover:bg-gray-50 transition duration-150">
                    <td class="px-6 py-4 whitespace-nowrap">
                        <div class="flex items-center gap-3">
                            <div>
                                <div class="text-sm font-bold text-gray-900"><?php echo $buyer['nama_lengkap']; ?></div>
                            </div>
                        </div>
                    </td>
                    ...
                    <td class="px-6 py-4 text-sm text-gray-600 max-w-xs truncate">
                        <?php echo $buyer['no_hp']; ?>
                    </td>
                    <td class="px-6 py-4 text-sm text-gray-600 max-w-xs truncate">
                        <?php echo $buyer['created_at']; ?>
                    </td>
                    <td class="px-6 py-4 whitespace-nowrap text-center">
                        <div class="flex items-center justify-center gap-2">
                            <a href="./?page=detail-user&id=<?php echo $buyer['id']; ?>" class="p-2 bg-blue-50 text-blue-600 rounded-lg hover:bg-blue-100 transition"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg></a>
                            <a href="./?page=edit-user&id=<?php echo $buyer['id']; ?>" class="p-2 bg-yellow-50 text-yellow-600 rounded-lg hover:bg-yellow-100 transition"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg></a>
                            <button onclick="confirmDelete(<?php echo $buyer['id']; ?>, '<?php echo $buyer['nama']; ?>')" class="p-2 bg-red-50 text-red-600 rounded-lg hover:bg-red-100 transition"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg></button>
                        </div>
                    </td>
                </tr>
                <?php endforeach; ?>
